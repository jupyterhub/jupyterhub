...
Inside the container, JupyterHub's working directory is `/srv/jupyterhub`.
Unless you pass a different config path on the command line, the Hub loads:

```
/srv/jupyterhub/jupyterhub_config.py
```

Cookie secrets and the default sqlite database are also stored under
`/srv/jupyterhub`.

### Mount a config file from the host
...
### Generate a config inside the container

```bash
docker exec -it jupyterhub bash
jupyterhub --generate-config
# edit /srv/jupyterhub/jupyterhub_config.py, then restart the container from the host:
