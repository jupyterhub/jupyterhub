...
Plase, note that the above IPs are formatted in IPv4, your setup may require IPv6 (discussed blelow).
...
#### Login issues when running on Kubernetes

If your JupyterHub is running on top of Kubernetes, it is very likely that your cloud provider sets-up a Load Balancer or
other networking services outside of JupyterHub. The issue may also arise, when you are setting up and Ingress resource
through Helm Chart. In this case, setting `JUPYTERHUB_XSRF_ANONYMOUS_IP_CIDRS="0.0.0.0/0"` may not be enough.
This may be especially the case, when you see that the Hub pod receives request from different IP like `::ffff:xx.xxx.xx.xx`,
even though request come from the same user, who tries to log in. The `::ffff:` is the representation of IPv4 address in the
IPv6 language.
The different IPs actually come from the Load Balancer
or Ingress that redirects the requests. In order to resolve the issue, you should set:
...
This setup includes the IP addresses in the IPv6 system `::/0`. If you know what IP ranges are generated by the Ingress
or external Load Balancer (setup in cloud provider), then You can restrict the IP ranges for additional security
measures.
