...
class BaseHandler(RequestHandler):
    ...
    def get_next_url(self, user=None, default=None):
        ...
        if (next_url + '/').startswith(
            (
                '{}://{}/'.format(self.request.protocol, self.request.host),
                '//%s/' % self.request.host,
            )
        ) or (
            self.subdomain_host
    ...
    async def auth_to_user(self, authenticated, user=None):
        ...
        if user and username != user.name:
            raise ValueError("Username doesn't match! {} != {}".format(username, user.name))
    ...
    async def spawn_single_user(self, user, server_name='', options=None):
        ...
        if server_name:
            user_server_name = '{}:{}'.format(user.name, server_name)
        ...
        if server_name in user.spawners and user.spawners[server_name].pending:
            ...
            raise RuntimeError("{} pending {}".format(user_server_name, pending))
    ...
    async def stop_single_user(self, user, server_name=''):
        ...
        spawner = user.spawners[server_name]
        if spawner.pending:
            raise RuntimeError("{} pending {}".format(spawner._log_name, spawner.pending))
