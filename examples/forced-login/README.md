...
If your application chooses to launch the server and wait for it to be ready before redirecting, you can skip all JupyterHub UI, and only have the UI for your application and their running environment.
[API only mode](https://jupyterhub.readthedocs.io/en/stable/howto/api-only.html) is useful for situations like this, where you want to keep JupyterHub as a hidden implementation detail.
BinderHub (https://mybinder.org) is an example of such an application, where JupyterHub doesn't serve a single page to users.

This directory contains a custom Service (an example of your external application) and a ForcedLoginAuthenticator, which
Users visit the application, which is a mock application that only lets you enter a username and path to land on.
When users enter their username, the application allocates a random token and redirects the user to `/hub/login?token=...`.
The ForcedLoginAuthenticator handles this login request and exchanges the token for a username.
Once the token has been used once, it cannot be used again.
This is a way to have 'forced login' (login via a simple link), but which is not reusable.
